...
ALL_NODE_VERSIONS=$(curl -L --silent https://unofficial-builds.nodejs.org/download/release/ | pup 'a text{}' | grep -o '[0-9]\+.[0-9]\+.[0-9]\+' | sort --version-sort -r);
for PHP_VERSION in $PHP_VERSIONS; do
    RELEASES=$(curl -L --silent 'https://www.php.net/releases?json&version='$PHP_VERSION)
    exact_php_version=$(echo ${RELEASES} | jq -r '."version"')
